...
describe('HmUtil', () => {
  ...
  describe('getIpAddressFromReqHeader', () => {
    ...
    test('エッジケース: socket.remoteAddressからカンマ区切りIPを取得できること', () => {
      const req = {
        headers: {},
        socket: {
          remoteAddress: '192.168.1.100, 10.0.0.1',
        },
      };
    ...
    test('エッジケース: socketはあるがremoteAddressがない場合、空文字列を返すこと', () => {
      const req = {
        headers: {},
        socket: {},
      };
    ...
    test('エッジケース: headersもsocketもない場合、空文字列を返すこと', () => {
      const req = {
        headers: {},
      };
  ...
  describe('checkIpAddressIncludedList', () => {
    ...
    test('エッジケース: 複数の*を含むIPアドレス範囲でCIDR表記に変換されること', () => {
      const req_ip = '192.168.10.100';
      const ip_list = ['192.168.*.*_LAN'];

      const result = hmUtil.checkIpAddressIncludedList(req_ip, ip_list);
      expect(result).toBe(true);
    });

    test('エッジケース: *が4つある場合（*.*.*.*）、すべてのIPアドレスにマッチすること', () => {
      const req_ip = '10.20.30.40';
      const ip_list = ['*.*.*.*_All'];
