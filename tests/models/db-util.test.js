...
describe('DbUtil', () => {
  ...
  describe('constructor', () => {
    test('正常系: db_configが既に設定されている場合、再初期化されないこと', () => {
      // 1回目のrequireで既にdb_configが設定されている
      const dbUtil1 = require('../../models/db-util');

      // 2回目のrequireでは、constructorのearly returnが実行される
      // （db_config !== nullなのでreturnされる）
      const dbUtil2 = require('../../models/db-util');

      // 同じインスタンスが返されることを確認
      expect(dbUtil1).toBe(dbUtil2);
    });
  });

  describe('executeQueryWrite', () => {
    test('正常系: 書き込みクエリで結果が空でない場合も正常に動作すること', async () => {
      const query = 'UPDATE users SET name = $1 WHERE id = $2';
      const params = ['Updated Name', 'user001'];
      const funcName = 'testFunction';
      const mockResult = {
        error: false,
        body: {
          rows: [{ id: 'user001', name: 'Updated Name' }],
          rowCount: 1,
        },
      };
