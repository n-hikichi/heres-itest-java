...
describe('Telework', () => {
  ...
  describe('insertTeleworkLog', () => {
    ...
    test('エッジケース: 想定外のcategoryでdefaultケースが実行されること', async () => {
      const workinghourId = 'wh001';
      const companyId = 'company001';
      const payload = {
        staff_id: 'staff001',
        category: 99, // 想定外のカテゴリ (1-4以外)
        date: '2025-10-28',
        time: '09:00',
        lat: '35.6812',
        lon: '139.7671',
      ...
      // 最新テレワークデータのモックで、想定外のcategoryを返す
      dbUtil.executeQueryRead.mockResolvedValue({
        error: false,
        status: 'OK',
        body: [{
          category: 5, // 想定外のカテゴリ（1-4以外）
          input_date: '20251027',
          input_hour: '1800',
          linked_date: '20251027',
        }],
    ...
    test('異常系: 直近が「勤務開始」で「着席」を登録しようとした場合、不整合エラーを返すこと', async () => {
      const workinghourId = 'wh001';
      const companyId = 'company001';
      const payload = {
        staff_id: 'staff001',
    ...
    test('正常系: 日付がYYYYMMDD形式（区切り文字なし）の場合、正しく処理できること', async () => {
      const workinghourId = 'wh001';
      const companyId = 'company001';
      const payload = {
        staff_id: 'staff001',
        category: 1, // 勤務開始
        date: '20251028', // 区切り文字なし
        time: '09:00',
        lat: '35.6812',
        lon: '139.7671',
        comment: '勤務開始',
  ...
  describe('insertTeleworkLogByMember', () => {
    ...
    // 【カバレッジに関する重要な注記】
    // ========================================================================
    // 以下のテストケースにより、models/telework.js の insertTeleworkLogByMember メソッドを
    // テストしていますが、Lines 251, 257-279 は単体テストでのカバーが技術的に困難です。
    //
    // 【未カバー行】
    // - Line 251: updated = TELEWORK_DATA_INITIAL
    // - Lines 257-279: linked_date未指定時の複雑な処理
    //   - 最新データ取得
    //   - 日またぎ判定
    //   - エラーハンドリング
    //
    ...
    //
    // 【対応方針】
    // - これらの行は統合テストで検証することを推奨
    // - 単体テストでは現在のカバレッジ（96.31%）で十分な品質を確保
    // - 関連するエッジケースは以下のテストで部分的にカバー済み：
    //   - updated未指定のみ（linked_date指定あり）
    //   - linked_date未指定のみ（updated指定あり）
    //   - updated/linked_date両方未指定のテストは追加したがモック制約によりカバー不可
    //
    // 詳細: CHANGELOG.md, TEST_COVERAGE_SUMMARY_c0c1.md 参照
    // ========================================================================
    ...
    test('エッジケース: updatedパラメータが未指定の場合、TELEWORK_DATA_INITIALが設定されること', async () => {
      const workinghourId = 'wh001';
      const companyId = 'company001';
      const payload = {
        staff_id: 'staff001',
    ...
    test('エッジケース: linked_dateが未指定で日またぎ勤務の場合、直近のlinked_dateが使用されること', async () => {
      const workinghourId = 'wh001';
      const companyId = 'company001';
      const payload = {
        staff_id: 'staff001',
        category: 2, // 離席（勤務開始以外）
        date: '2025-10-29', // 翌日
        time: '01:00',
        lat: '35.6812',
        lon: '139.7671',
        comment: '深夜勤務',
        updated: 0,
        // linked_date を省略（undefinedとなる）
      };

      // 最新テレワークデータのモック（前日の勤務開始）
      dbUtil.executeQueryRead.mockResolvedValue({
        error: false,
        status: 'OK',
        body: [{
    ...
    test('エッジケース: updated/linked_date両方未指定で日またぎ勤務の場合', async () => {
      const workinghourId = 'wh001';
      const companyId = 'company001';
      const payload = {
        staff_id: 'staff001',
    ...
    test('エッジケース: updated/linked_date両方未指定で同日勤務の場合', async () => {
      const workinghourId = 'wh001';
      const companyId = 'company001';
      const payload = {
        staff_id: 'staff001',
