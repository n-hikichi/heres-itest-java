...
describe('LoginAuth', () => {
  ...
  describe('authenticate', () => {
    ...
    // ========================================================================
    //
    // 以下のテストケースは、外部コマンド（child_process.exec）を使用した
    // Java EncDecクラスによるパスワード暗号化/復号化処理に依存しています。
    //
    // 【単体テストでの制約】
    // - 外部プロセス（Java実行環境）が必要
    // - child_process.execのモック化が複雑
    // - タイムアウトエラーが発生（10秒超過）
    //
    // 【統合テスト実施方針】
    // プロジェクトの方針として、外部コマンド実行を含む処理は統合テストで検証します。
    // 単体テストでは、モックを使用した最小限のテストのみ実施します。
    //
    // 【影響範囲】
    // - models/login-auth.js:83 - exec(java_cmd) による復号化処理
    // - models/login-auth.js:84-128 - パスワード復号化～認証処理全体
    //
    // 【統合テスト環境要件】
    // - Java実行環境（JDK）
    ...
    //
    // 【該当するテストケース】
    // 1. "正常系: 正しいID/パスワードで認証が成功すること"（89行目）
    // 2. "異常系: パスワードが一致しない場合、nullを返すこと"（182行目）
    //
